...
public class GenericJsonRepository<T> where T : IEntidade
{
    ...
    private List<T> CarregarDoArquivo()
    {
        ...
        return JsonSerializer.Deserialize<List<T>>(json, ObterOpcoesJson()) ?? new List<T>();
    }

    private void SalvarNoArquivo()
    {
        var json = JsonSerializer.Serialize(_entidades, ObterOpcoesJson());
        File.WriteAllText(_caminhoArquivo, json);
    }

    private JsonSerializerOptions ObterOpcoesJson()
    {
        return new JsonSerializerOptions
        {
            ...
            // Remova ou comente a linha abaixo se não estiver usando .NET 8+
            // TypeInfoResolver = System.Text.Json.Serialization.Metadata.DefaultJsonTypeInfoResolver.Shared
        };
    }
}
